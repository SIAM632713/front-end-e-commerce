        ...
        <div>
            <div className="bg-white rounded shadow-md">
                <div className="flex justify-between items-center p-4 border-b">
                    ...
                    <button className="bg-blue-500 text-white px-4 py-1 rounded text-sm hover:bg-blue-600">SEE ALL
                    </button>
                </div>
                <table className="w-full text-left border-collapse">
                    ...
                    {userData.map((item, index) => (
                        <tr key={index} className="border-b hover:bg-gray-50">
                            ...
                            <td className="p-3">
                                    <span
                                        className={`px-2 py-1 rounded-full text-xs font-semibold ${item.role === 'admin' ? 'bg-purple-500 text-white' : 'bg-yellow-400 text-black'}`}>
                                        {item.role}
                                    </span>
                            </td>
                            <td onClick={()=>HandleModalopen(item)} className="p-3 text-blue-600 cursor-pointer flex items-center gap-1">
                                <Pencil size={14}/> Edit
                            </td>
                            <td className="p-3">
            ...
            <UserInputstatus HandleModalclose={HandleModalclose} isModalOpen={isModalOpen} user={selectedUser}/>
        </div>
    );
};
